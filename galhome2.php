    ...
    if(empty($error)==true){
        ...
        if(move_uploaded_file($file_tmp,$image_file)){
           // $con= new mysqli("localhost","mygaller", "65e1l4JHuc", "mygaller_test");
            ...
            /*
           $sqli="select path from upload_image where userId=".$userId;
...
<form class="row" method="post" action="/myGallery.com/galhome1.php" autocomplete="off">

    <div class="col-sm-12 form-group">
        <button type="submit" name="view" class="btn btn-default pull-right">send</button>
    </div>
</form>
<!-- about company page -->
 ...
 if(isset($_POST['view'])){
     $sqli = "select path from upload_image where userId=".$_SESSION['user'];
     ...
     if ($result->num_rows > 0) {
         while ($row = $result->fetch_assoc()) {
             ...
             ?>
                     ...
                     echo "<img src='$path'  height='408' width='306'/>";
